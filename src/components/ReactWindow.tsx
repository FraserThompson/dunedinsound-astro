		...
		<div id={id} className={`${className} ${reactWindowWrapper}`}>
			{(search || sort || filter) && <div class={shuffleFilter}>
				...
				{filter && <div className={filtersWrapper}>
					...
					(filter.type !== 'select'
						?
						<label style={{ cursor: "pointer", textWrapMode: 'nowrap' }}>
							...
							<span className={filter.checkedTitle ? 'whenUnchecked' : ''} style={{ textWrapMode: 'nowrap' }}>{filter.icon ? getIcon(filter.icon) : filter.title}</span>
							{filter.checkedTitle && <span className='whenChecked' style={{ textWrapMode: 'nowrap' }}>{filter.checkedTitle}</span>}
						</label>
						:
						<label class="flex">
