...
If you're on a desktop, maybe you noticed everything is smaller. If you're on mobile, the changes are less major, but hopefully you'll notice the easier navigation.

And no matter what device you're on you should notice everything is a lot faster.

This is the result of two months laboring in the code mines to move my website from Gatsby to Astro. I believe I encountered and solved some fairly unique problems since there aren't really many (or any?) other Gatsby/Astro sites with hundreds of gigabytes of images.
...
I also had high hopes for the image processing system, a major feature of Gatsby. My site has over 20,000 photos, and I wanted to process and serve these at optimal resolutions for all devices, to help speed up the client experience, and also save hosting costs. 
...
First it became clear my site (with over 20,000 images, many 100mb+ MP3 files, and over 1,000 pages) was too much for Gatsby, at least in the way they intended it to be used. Over the years, my build times became longer and longer, reaching multiple hours. But thanks to Gatsby being open source, I was able to identify the cause and contribute a fix, which was then [rolled into version 5.5](https://www.gatsbyjs.com/docs/reference/release-notes/v5.5/) and drastically sped up my build times.
...
But as of earlier this year my build time was back to like four hours. Which was pretty annoying and really hurt my dev workflow.
...
Astro has [documentation on migrating Gatsby to Astro](https://docs.astro.build/en/guides/migrate-to-astro/from-gatsby/) to help all the people who, like me, want to move away from a dead platform. Initially I followed this documentation, but it soon became clear that the process was a lot less simple than the docs suggested, and I ended up basically just rewriting everything.
...
An example of this was my Artists page component, which basically just wrapped a JS library ([ShuffleJS](https://vestride.github.io/Shuffle/)) in React. I replaced this with a custom web component which wraps that same library but in a tidier way without all the React cruft.
...
The lesson I learnt from Gatsby is that static site generators are not intended to handle this many large files. Build directories are meant to be ephemeral and deletable without a big hassle, and having to re-generate tens of thousands of images is a big hassle. Also build systems sometimes to hash files to determine if they've changed, and if you have 100mb+ MP3s this can cause problems.

So I decided it would be better to run a secondary task to process all my images, and put the processed versions in a directory which Astro doesn't know about. This feels like added complexity, but I'm not sure if that's true.
...
I tested this on a subset of media and found it to be very fast, much faster than Gatsby which tended to linger on “processing images” and doing *something*. Even on a mechanical disk it took minutes to check existing files for changes, and not much longer to generate proxies.
...
It's nice to know I have full control of this pipeline and can optimize it for my circumstances as opposed to Gatsby where I was at the whims of the open source community. It's already significantly faster at processing images.
...
When I first designed the website, I was more motivated by what other sites did and what was "standard" web design. So I went for the usual mobile first web UX where everything is big, and most effort is put into the mobile experience. And this makes sense because mobile users are the majority. 

However, different types of users are in different contexts eg. sitting at a computer vs on the couch with a device which can go in/out of your pocket, and apps which are begging for your attention. So desktop users probably have more time, are more likely to be more focused, and so I should provide them with tools which fit that context, even if they're a minority.
...
Removing the large files from my public directory so neither Astro or Vite knew about them at all reduced the build down to 15 minutes. So it became clear that my original plan of storing my files in the public directory wasn't going to work.
...
My new dist directory is ~100gb, compared to Gatsby's ~260gb (some of this will also be because the full resolution JPGs are now compressed with MozJPEG) which will save a bit of money.

<Image size="blog" responsiveImage={frontmatter.responsiveImages['deleted']} alt="Deleting my old S3 bucket"/>

## The result

The resulting site is, I believe, easier to use, significantly faster, and will be much easier to dev on.

I meant to measure the difference in payload size too but I forgot. But what's cool is how some pages require no JavaScript, or very little. The homepage for example contains no React components so it only sends 5kb of JavaScript. In contrast, every page on my Gatsby site shipped a JavaScript bundle which included React.
...
Basically I really like Astro. It feels just thin enough to be useful instead of cumbersome, it doesn't get in the way when it shouldn't, and it provides a platform to use evolving web standards as opposed to some persons library which they will get bored of and stop maintaining in a few years. I just hope the "thinness" is a deliberate design decision rather than because it's new and immature.
